#!/usr/bin/env python3
"""
Final Optimized AML Training
===========================

This script fixes the NaN loss issue while maintaining high performance.
"""
...
print("🚀 Final Optimized AML Training")
print("=" * 60)

class StableFocalLoss(nn.Module):
    """Stable Focal Loss with numerical stability"""
    ...
    def forward(self, inputs, targets):
        ...
        ce_loss = torch.clamp(ce_loss, min=1e-7, max=1e7)
        ...
        # Clamp final loss
        focal_loss = torch.clamp(focal_loss, min=1e-7, max=1e7)
...
def load_optimized_dataset():
    """Load optimized dataset for stable training"""
    print("📊 Loading optimized dataset...")
    
    data_path = "/content/drive/MyDrive/LaunDetection/data/raw"
    
    # Load a reasonable sample size for stable training
    print("Loading 500K transactions for stable training...")
    ...
    print(f"📊 Optimized Dataset AML Distribution:")
...
def create_stable_graph(transactions):
    """Create a stable graph for training"""
    print("🕸️  Creating stable graph...")
    ...
    for account in all_accounts:
        ...
        account_features[account] = features.tolist()
        G.add_node(account, features=features.tolist())
    ...
    for _, transaction in balanced_transactions.iterrows():
        ...
        if from_acc in G.nodes and to_acc in G.nodes:
            # Create normalized edge features
            edge_features = [
...
def create_stable_training_data(graph_data, num_samples=4000):
    ...
    # Create training graphs with stable sampling
    training_graphs = []
    aml_graphs = 0
    non_aml_graphs = 0
...
def train_final_optimized_model():
    ...
    transactions = load_optimized_dataset()
    
    if transactions is None:
        print("❌ Failed to load optimized dataset")
        return
    ...
    # Create stable training data
    training_graphs = create_stable_training_data(graph_data, num_samples=4000)
    ...
    # Create data loaders
    train_loader = DataLoader(train_graphs, batch_size=8, shuffle=True)  # Smaller batch size
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=0.0001, weight_decay=1e-5)  # Lower LR
    ...
    for epoch in range(50):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch+1}"):
            ...
            # Backward pass
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=0.5)  # Smaller clip
            optimizer.step()
    ...
    print(f"✅ Final optimized training completed! Best F1: {best_f1:.4f}")
    
    if best_f1 > 0.7:
        print("🎉 EXCELLENT! Final model is learning effectively")
        print("   Ready for production use!")
    elif best_f1 > 0.5:
        print("✅ GOOD! Final model is learning moderately")
        print("   Consider increasing dataset size for better performance")
    else:
        print("⚠️  Final model needs improvement. Consider:")
