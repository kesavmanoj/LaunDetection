#!/usr/bin/env python3
"""
Production-Ready AML Training
=============================

Fixes all overfitting issues with:
- Simplified architecture (single branch)
...
print("🚀 Production-Ready AML Training")
print("=" * 50)
...
def load_production_data():
    """Load data for production training"""
    ...
    for dataset in datasets:
        ...
        if os.path.exists(file_path):
            ...
            try:
                ...
                data = pd.read_csv(file_path, nrows=100000)  # Limit for memory
                ...
                print(f"      ✅ {dataset}: {len(data):,} transactions, {data['Is Laundering'].mean()*100:.2f}% AML")
    ...
    print(f"✅ Combined dataset: {len(combined_data):,} transactions")
...
def create_production_features(data):
    """Create production-ready features (NO AML FLAGS)"""
    print("\n🔄 Creating production-ready features...")
...
def train_production_model(train_data, val_data, epochs=100):
    """Train production model with early stopping"""
    ...
    for epoch in range(epochs):
        ...
        if val_f1 > best_val_f1:
            ...
            patience_counter = 0
            # Save best model
            torch.save(model.state_dict(), '/content/drive/MyDrive/LaunDetection/models/production_model.pth')
        else:
            patience_counter += 1
...
def evaluate_production_model(model, test_data):
...
def main():
    ...
    try:
        ...
        # Create features
        x, edge_index, edge_attr, y_true = create_production_features(data)
        
        # Create proper splits
        train_data, val_data, test_data = create_proper_splits(x, edge_index, edge_attr, y_true)
        
        # Train model
        model = train_production_model(train_data, val_data)
        
        # Evaluate model
        results = evaluate_production_model(model, test_data)
        
        print("\n🎉 PRODUCTION TRAINING COMPLETE!")
        print("=" * 50)
        print("✅ Model trained with proper validation")
