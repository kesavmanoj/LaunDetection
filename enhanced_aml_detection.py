#!/usr/bin/env python3
"""
Enhanced AML Detection Training
==============================

This script focuses specifically on improving AML detection performance
by using advanced techniques for imbalanced classification.
"""
...
print("🎯 Enhanced AML Detection Training")
print("=" * 50)

class EnhancedAMLGNN(nn.Module):
    """Enhanced GNN specifically designed for AML detection"""
    def __init__(self, input_dim, hidden_dim=128, output_dim=2, dropout=0.2):
        super(EnhancedAMLGNN, self).__init__()
        
        # Enhanced architecture for AML detection
    ...
    def forward(self, x, edge_index, edge_attr=None):
        ...
        if self.edge_classifier is None:
            ...
            print(f"   Creating enhanced edge classifier with input_dim={actual_input_dim}")
...
class EnhancedAMLTrainer:
    """Enhanced trainer specifically for AML detection"""
    ...
    def train_enhanced_aml_model(self, data, epochs=300, learning_rate=0.0005):
        """Train enhanced model specifically for AML detection"""
        print("🎯 Starting Enhanced AML Detection Training...")
        
        # Create model with correct input dimension
        actual_input_dim = data.x.shape[1]
        print(f"   📊 Actual input dimension: {actual_input_dim}")
        
        self.model = EnhancedAMLGNN(
            input_dim=actual_input_dim,
            hidden_dim=128,  # Larger hidden dim for better representation
            output_dim=2,
            dropout=0.2  # Lower dropout for better learning
        ).to(self.device)
        
        print(f"✅ Enhanced model created with {sum(p.numel() for p in self.model.parameters()):,} parameters")
        
        # Setup training with AML-focused parameters
        optimizer = torch.optim.AdamW(self.model.parameters(), lr=learning_rate, weight_decay=1e-4)
        scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=50, T_mult=2)
        
        # Use Focal Loss for better AML detection
        focal_loss = FocalLoss(alpha=2, gamma=2)
        
        # Also compute class weights for additional balancing
        ...
        print(f"   🎯 Training for {epochs} epochs with Focal Loss...")
        ...
        for epoch in range(epochs):
            ...
            # Use both Focal Loss and Weighted CrossEntropy
            focal_loss_val = focal_loss(out, data.y)
            weighted_loss_val = weighted_criterion(out, data.y)
            
            # Combine losses
            loss = 0.7 * focal_loss_val + 0.3 * weighted_loss_val
            ...
            if epoch % 5 == 0:
                ...
                with torch.no_grad():
                    ...
                    if aml_f1 > best_aml_f1:
                        ...
                        # Save best model
                        torch.save(self.model.state_dict(), '/content/drive/MyDrive/LaunDetection/enhanced_aml_model.pth')
                        print(f"   💾 Saved best AML model (AML F1={best_aml_f1:.4f})")
                    else:
                        patience_counter += 1
        ...
        print(f"\n✅ Enhanced AML Training Complete! Best AML F1: {best_aml_f1:.4f}")
        return self.model, best_aml_f1
    
    def evaluate_enhanced_model(self, data):
        """Evaluate the enhanced AML model"""
        print("📊 Evaluating Enhanced AML Model...")
        ...
        print("\n📊 Enhanced AML Model Evaluation Results:")
        print("=" * 50)
...
def main():
    """Main enhanced AML detection pipeline"""
    print("🎯 Starting Enhanced AML Detection Training...")
    
    # Initialize trainer
    trainer = EnhancedAMLTrainer()
    ...
    # Train enhanced AML model
    model, best_aml_f1 = trainer.train_enhanced_aml_model(data)
    
    # Evaluate model
    metrics, aml_metrics, cm = trainer.evaluate_enhanced_model(data)
    
    print("\n🎉 Enhanced AML Detection Complete!")
    print("=" * 50)
    print("✅ Enhanced model trained with AML-focused techniques")
    print("✅ Focal Loss and advanced balancing applied")
    print("✅ Better AML detection performance expected")
