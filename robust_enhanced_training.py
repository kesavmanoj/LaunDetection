#!/usr/bin/env python3
"""
Robust Enhanced AML Multi-GNN Training
======================================

This script implements robust training with:
- Larger dataset (50K+ transactions)
...
print("🚀 Robust Enhanced AML Multi-GNN Training")
print("=" * 60)
...
def load_larger_dataset():
    """Load a much larger dataset for training"""
    print("📊 Loading larger dataset...")
    
    # Try to load from enhanced preprocessing with larger sample
    processed_path = "/content/drive/MyDrive/LaunDetection/data/processed"
    
    # Check if we have a larger preprocessed dataset
    larger_graph_file = os.path.join(processed_path, "larger_graph_created.pkl")
    
    if os.path.exists(larger_graph_file):
        print("✅ Found larger preprocessed dataset")
        with open(larger_graph_file, 'rb') as f:
            return pickle.load(f)
    
    # If not, create a larger dataset from scratch
    print("⚠️  Creating larger dataset from scratch...")
    return create_larger_dataset()

def create_larger_dataset():
    """Create a larger dataset with better balance"""
    print("🔧 Creating larger dataset with 50K transactions...")
    ...
    try:
        ...
        accounts = pd.read_csv(os.path.join(data_path, 'HI-Small_accounts.csv'), nrows=10000)
        
        print(f"✅ Loaded {len(transactions)} transactions, {len(accounts)} accounts")
        
        # Create better graph with more connections
        return create_robust_graph(transactions, accounts)
        
    except Exception as e:
        print(f"❌ Error loading data: {e}")
        return create_synthetic_larger_dataset()

def create_robust_graph(transactions, accounts):
    """Create a more robust graph with better connectivity"""
    print("🕸️  Creating robust graph with better connectivity...")
    ...
    for _, account in accounts.iterrows():
        ...
        features.extend(np.random.random(10).tolist())
    ...
    for _, transaction in transactions.iterrows():
        ...
        if from_acc in G.nodes and to_acc in G.nodes:
            ...
            # Add random features
            edge_features.extend(np.random.random(7).tolist())
            
            G.add_edge(from_acc, to_acc, features=edge_features, label=transaction.get('Is Laundering', 0))
            edges_added += 1
    
    print(f"✅ Created robust graph: {G.number_of_nodes()} nodes, {G.number_of_edges()} edges")
...
def create_synthetic_larger_dataset():
    """Create a synthetic larger dataset for testing"""
    ...
    for i in range(1000):
        ...
        for _ in range(num_connections):
            ...
            if target != i:
                ...
                label = 1 if np.random.random() < 0.1 else 0  # 10% AML
...
def create_training_data_robust(graph_data, num_samples=1000):
...
def train_robust_model():
    ...
    # Load larger dataset
    graph_data = load_larger_dataset()
    ...
    training_graphs = create_training_data_robust(graph_data, num_samples=2000)
