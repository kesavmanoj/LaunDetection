#!/usr/bin/env python3
"""
Hyperparameter Optimization for AML Detection
============================================

This script performs comprehensive hyperparameter optimization for the edge-level
AML detection model using grid search and random search strategies.
"""
...
print("🔧 Hyperparameter Optimization for AML Detection")
print("=" * 60)

class OptimizedEdgeLevelGNN(nn.Module):
    """Optimized GNN for edge-level classification with configurable architecture"""
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers=3, dropout=0.1, 
        ...
        super(OptimizedEdgeLevelGNN, self).__init__()
...
def train_model_with_config(data, config, device, max_epochs=50):
    """Train model with specific configuration"""
    model = OptimizedEdgeLevelGNN(
        input_dim=15,
...
def grid_search_optimization():
    ...
    for i, combination in enumerate(tqdm(combinations, desc="Grid Search")):
        config = dict(zip(keys, combination))
        
        try:
            score = train_model_with_config(data, config, device, max_epochs=30)
            ...
            print(f"Config {i+1}/{len(combinations)}: F1={score:.4f}")
            
        except Exception as e:
            print(f"Config {i+1}/{len(combinations)}: FAILED - {str(e)}")
            continue
...
def main():
    ...
    # For now, use random search as it's more efficient
    print("🎲 Using Random Search for efficiency...")
    ...
    with open('/content/drive/MyDrive/LaunDetection/hyperparameter_results.pkl', 'wb') as f:
        pickle.dump(results_data, f)
    
    print(f"✅ Results saved to hyperparameter_results.pkl")
