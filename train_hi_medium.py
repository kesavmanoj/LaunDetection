#!/usr/bin/env python3
"""
Train Model on HI-Medium Dataset Only
=====================================

Trains the matching model specifically on HI-Medium dataset.
"""
...
print("🚀 HI-Medium Dataset Training")
print("=" * 50)

class HIMediumAMLGNN(nn.Module):
    """AML GNN model for HI-Medium dataset"""
    def __init__(self, input_dim=15, hidden_dim=256, output_dim=2, dropout=0.1):
        super(HIMediumAMLGNN, self).__init__()
...
def load_hi_medium_dataset():
    """Load HI-Medium dataset with memory management"""
    print("📊 Loading HI-Medium dataset...")
    ...
    max_transactions = 1000000  # 1M transactions max
    if len(clean_transactions) > max_transactions:
        ...
        print(f"   ⚠️ Limited to {max_transactions:,} transactions for memory management")
    
    # Limit AML transactions
    max_aml = 5000  # 5K AML transactions max
    ...
    if len(aml_transactions) > max_aml:
        ...
        print(f"   ⚠️ Limited to {max_aml:,} AML transactions")
    
    print(f"✅ HI-Medium: {len(clean_transactions):,} transactions")
...
def create_balanced_training_data(data, target_aml_rate=0.10):
    """Create balanced training data from HI-Medium"""
    print(f"\n🔄 Creating balanced training data ({target_aml_rate*100:.1f}% AML rate)...")
    ...
    # Combine and shuffle
    training_data = pd.concat([aml_transactions, non_aml_sample])
    training_data = training_data.sample(frac=1, random_state=42).reset_index(drop=True)
    
    actual_aml_rate = training_data['Is Laundering'].mean()
    print(f"✅ Balanced training dataset:")
...
def train_hi_medium_model():
    """Train model on HI-Medium dataset"""
    print("🚀 Starting HI-Medium training...")
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Using device: {device}")
    
    # Load HI-Medium dataset
    data = load_hi_medium_dataset()
    if data is None:
        print("❌ Cannot load HI-Medium dataset. Training aborted.")
        return None
    
    # Create balanced training data
    training_data = create_balanced_training_data(data, target_aml_rate=0.10)
    ...
    # Create model
    model = HIMediumAMLGNN(input_dim=15, hidden_dim=256, output_dim=2, dropout=0.1).to(device)
        ...
        if (epoch + 1) % 10 == 0:
            ...
            with torch.no_grad():
                ...
                if f1_aml > best_aml_f1:
                    ...
                    model_path = f'{models_dir}/hi_medium_aml_model.pth'
...
def main():
    ...
    try:
        model_path = train_hi_medium_model()
        if model_path:
            print("\n🎉 HI-MEDIUM TRAINING COMPLETE!")
            print("=" * 50)
